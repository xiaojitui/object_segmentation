...
class Trainer(object):
    ...
    def train(self, X_train, y_train, X_test, y_test, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1,
        ...
        with tf.Session() as sess:
            ...
            idx_t = np.random.choice(len(X_test), self.verification_batch_size, replace = False)
            test_x, test_y = X_test[idx_t], y_test[idx_t]
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch * training_iters), ((epoch + 1) * training_iters)):
                    idx_u = np.random.choice(len(X_train), self.batch_size, replace = False)
                    batch_x, batch_y = X_train[idx_u], y_train[idx_u]
